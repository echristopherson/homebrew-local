...
class MitScheme < Formula
  ...
  head 'git://git.savannah.gnu.org/mit-scheme.git', :branch => 'master'
  ...
  def install
    ...
    prefix_substitutions = %w(6001/edextra.scm 6001/floppy.scm compiler/etc/disload.scm configure
    edwin/techinfo.scm edwin/unix.scm
    swat/c/tk3.2-custom/Makefile swat/c/tk3.2-custom/tcl/Makefile swat/scheme/other/btest.scm)
    prefix_substitutions += %w(lib/include/configure lib/include/option.c) unless build.head?
    inreplace(prefix_substitutions) do |s|
    ...
    system "etc/make-liarc.sh", "--disable-debugging", "--prefix=#{prefix}", "--mandir=#{man}"
    system "make install"
  end
end
